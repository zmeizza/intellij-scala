    ...
    if (conforms(t1, t2, checkWeak)) t2
      ...
      def lubWithExpandedAliases(t1: ScType, t2: ScType): ScType = {
        (t1, t2) match {
          ...
          case (TypeParameterType(_, Nil, _, upper), _) if !stopAddingUpperBound => lub(upper, t2, checkWeak)
          case (_, TypeParameterType(_, Nil, _, upper)) if !stopAddingUpperBound => lub(t1, upper, checkWeak)
          case (ScExistentialArgument(name, args, lower, upper), ScExistentialArgument(_, _, lower2, upper2))
