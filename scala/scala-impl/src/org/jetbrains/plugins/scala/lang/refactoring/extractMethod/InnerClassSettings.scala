...
import org.jetbrains.plugins.scala.lang.psi.types.TypePresentationTransformer
...
case class InnerClassSettings(needClass: Boolean, className: String, outputs: Array[ExtractMethodOutput], isCase: Boolean) {
  def classText(canonTextForTypes: Boolean): String = {
    def paramText(output: ExtractMethodOutput) = {
      ...
      val cleanUpTransformer = TypePresentationTransformer.cleanUp
      val typeText = if (canonTextForTypes) tp.canonicalText(cleanUpTransformer) else tp.presentableText
