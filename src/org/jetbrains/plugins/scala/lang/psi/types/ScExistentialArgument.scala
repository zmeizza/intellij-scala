...
case class ScExistentialArgument(name: String, args: List[TypeParameterType], lower: ScType, upper: ScType)
  ...
  override def visitType[T](visitor: TypeVisitor[T]): T = visitor match {
    case v: ExistentialArgumentVisitor[T] => v.visitExistentialArgument(this)
    case _ => visitor.notSupported(this)
  }
}

trait ExistentialArgumentVisitor[T] extends api.TypeVisitor[T] {
  def visitExistentialArgument(tp: ScExistentialArgument): T = default(tp)
}
