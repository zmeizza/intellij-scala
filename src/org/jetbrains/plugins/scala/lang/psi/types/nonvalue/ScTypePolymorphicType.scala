...
case class ScTypePolymorphicType(internalType: ScType, typeParameters: Seq[TypeParameter]) extends NonValueType with ScalaType {
  ...
  override def visitType[T](visitor: TypeVisitor[T]): T = visitor match {
    case v: TypePolymorphicTypeVisitor[T] => v.visitTypePolymorphicType(this)
...
trait TypePolymorphicTypeVisitor[T] extends api.TypeVisitor[T] {
  def visitTypePolymorphicType(tp: ScTypePolymorphicType): T = default(tp)
}